...
class Socket final {
 ...
 public:
  ...
  size_t read(void* buffer, size_t length, const uint64_t timeout);
};
...
class Session final {
 private:
  ...
  Response send(const std::string& method, const std::vector<uint8_t>& body, std::chrono::milliseconds timeout = std::chrono::milliseconds{-1});
